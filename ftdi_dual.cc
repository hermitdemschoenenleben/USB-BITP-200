...
ftdi_dual::ftdi_dual (int id, int nr, link_type link) { 
...
delib::value_t ftdi_dual::ping_ (value_t count) { 
...
void ftdi_dual::write_ (data_width dw, address_t address, value_t value) { 
...
delib::value_t ftdi_dual::read_ (data_width dw, address_t address) { 
  address_t shift = address & (8 - int(dw));
  address_t modifier = ((1 << int(dw)) - 1) << shift;
...
void ftdi_dual::cmd (opcode cmd, address_t address, uint8_t *matrix, address_t columns, address_t rows) {
  uint8_t job_id = job_id_inc ();
