...
orig_delib::orig_delib (int id, int nr) { handle_ = DapiOpenModule (id, nr); }

delib::value_t orig_delib::ping_ (value_t count) { return DapiPing (handle_, count); }

void orig_delib::write_ (data_width dw, address_t address, value_t value) { 
  switch (dw) {
    case d1:
...
void orig_delib::write_ (address_t address, address_t adress_depth, address_t increments, void *buff, size_t buff_len) {
  DapiWriteMultipleBytes (handle_, address, adress_depth, increments, (unsigned char*)buff, buff_len);
}

delib::value_t orig_delib::read_ (data_width dw, address_t address) { 
  switch (dw) {
    case d1:
...
void orig_delib::read_ (address_t address, address_t adress_depth, address_t increments, void *buff, size_t buff_len) {
